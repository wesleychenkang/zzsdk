...
public class ImageCache {
    ...
    public static ImageCache getInstance(ParamChain env, ImageCacheParams cacheParams) {
        // See if we already have an ImageCache stored in RetainFragment
        ImageCache imageCache = env.get(TAG, ImageCache.class);

        // No existing ImageCache, create one and store it in RetainFragment
        if (imageCache == null) {
            imageCache = new ImageCache(cacheParams);
            env.add(TAG,imageCache);
        }
    ...
    public static File getExternalCacheDir(Context context) {
        ...
        if (getUsableSpace(Environment.getExternalStorageDirectory())<1024*100) {
        	//! SD卡空间不足
